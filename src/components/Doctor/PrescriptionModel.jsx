...
const style = {
  ...
  const handleSubmit = async () => {
    const formattedPrescriptions = prescriptions.map(p => `${p.medicine} - ${p.time} - ${p.count}`);
  ...
  >
    <Box sx={style}>
      <Box sx={{ backgroundColor: theme.palette.primary.main, p: 1, borderRadius: 1, mb: 2 }}>
          <Typography id="prescription-modal-title" variant="h6" component="h2" sx={{ color: 'white' }}>
            Prescription Details
          </Typography>
        </Box>
      <Typography id="prescription-modal-description" sx={{ mb: 2 }}>
        This prescription is from Dr. {doctor.name} at {doctor.currentWorkingHospital}, specialized in {doctor.expertise.name}.
      </Typography>
      <Table>
        <TableHead>
          <TableRow>
