...
import { addExpertise,expertise,editExpertise } from '../../services/admin/apiMethods'
  ...
  const saveCategory = async (id) => {
    try {
      ...
      if (response.status === 200) {
        ...
        console.error('Error:', response); // Log the entire response for debugging
        // Handle error accordingly
      }
    } catch (error) {
      console.error('Save category failed:', error); // Catch and log any errors during the process
    }
  };
...
//   }
  ...
  return (
    <div className="m-4">
      <h1 className="text-3xl font-bold mb-4" style={{ color: blueColor, textAlign: 'center' }}>CATEGORIES</h1>
        ...
        <Table aria-label="categories table">
          ...
          <TableBody>
            {categories.map((category) => (
              <TableRow key={category._id} style={{ backgroundColor: '#fff', '&:hover': { backgroundColor: '#f5f5f5' } }}>
                <TableCell>
