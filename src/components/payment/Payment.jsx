...
function PaymentProcess() {
  ...
  const handleProceed = async () => {
    ...
    if (selectedPaymentOption === "online") {
      try {
        const response = await axios.post(`${BASE_URL}/api/patient/create-payment`, data);
        ...
        const options = {
          ...
          handler: async (response) => {
            try {
              const result = await axios.post(`${BASE_URL}/api/patient/verify-payment`, {
                response,
                order,
              });
      ...
      <div className="bg-white rounded-lg shadow-lg p-8 flex justify-between flex-wrap">
        <div className="w-1/2">
          <div className="w-full mb-8">
            ...
            <p className="text-gray-700"><span className="font-semibold">Name:</span> DR {selectedDoctor?.name}</p>
            <p className="text-gray-700"><span className="font-semibold">Experience:</span> {selectedDoctor?.experienceYears} years</p>
            <p className="text-gray-700"><span className="font-semibold">Expertise:</span> {selectedDoctor?.expertise.name}</p>
          ...
          <div className="mb-4">
            ...
            <p className="text-gray-700"><span className="font-semibold">Date:</span> {selectedDate?.toLocaleDateString()}</p>
            <p className="text-gray-700"><span className="font-semibold">Time:</span> {selectedShift}</p>
          </div>
          <div className="mb-4">
            <h3 className="text-xl font-semibold text-blue-800 mb-2">Payment Details</h3>
            <p className="text-gray-700"><span className="font-semibold">Amount:</span> {selectedDoctor.Fee}</p>
          </div>
          <div className="mb-4">
        ...
        <div className="w-1/2 flex flex-col items-center justify-center">
          <img src={Profileholder} alt="" className="w-[350px] h-[200px] mb-6 hidden md:block" />
          <img src={ProfileHolder2} alt="" className="w-[350px] h-[200px] hidden md:block" />
        </div>
